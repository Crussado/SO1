...
void agente(void * _args)
{
  ...
  for (;;) {
    ...
    switch (caso) {
      case 0:
          sem_post(&args->fumadores[0]);
          sem_post(&args->tabaco);
          sem_post(&args->papel);
          break;
      case 1:
          sem_post(&args->fumadores[1]);
          sem_post(&args->fosforos);
          sem_post(&args->tabaco);
          break;
      case 2:
          sem_post(&args->fumadores[2]);
          sem_post(&args->papel);
          sem_post(&args->fosforos);
          break;
    }
...
void *fumador1(void *_arg) // tiene fosforo
{
  args_t *args = (args_t *) _arg;
  printf("Fumador 1: Hola!\n");
  for (;;) {
    sem_wait(&args->fumadores[0]);
...
void *fumador2(void *_arg) // tiene papel
{
  args_t *args = (args_t *) _arg;
  printf("Fumador 2: Hola!\n");
  for (;;) {
    sem_wait(&args->fumadores[1]);
...
void *fumador3(void *_arg) // tiene tabaco
{
  args_t *args = (args_t *) _arg;
  printf("Fumador 3: Hola!\n");
  for (;;) {
    sem_wait(&args->fumadores[2]);
    sem_wait(&args->papel);
    sem_wait(&args->fosforos);
    fumar(3);
    sem_post(&args->otra_vez);
...
int main()
{
  ...
  sem_init(&args->fumadores[0], 0, 0);
  sem_init(&args->fumadores[1], 0, 0);
  sem_init(&args->fumadores[2], 0, 0);
...
/* El deadlock se produce cuando el par de recursos no es destinado a un hilo, sino que uno de ellos
es destinado a un hilo "a" y el otro ingrediente faltante a un hilo "b". */
